...
def forward_measurement(
    ...
    outputs = np.zeros((len(input), len(weight) * np.shape(input)[1]//10), dtype=np.float32)
    for i in range(len(input)):
        for j in range(len(weight)):
            input_to_dnpu = np.zeros(
                (7, len(input[i]) + 2 * SLOPE_LENGTH + REST_LENGTH)
            )
            ext_weights = np.insert(weight[j], DNPU_INPUT_ELECTRODE, 0.)
            # Setting electrodes
            for m in range(7):
                if m != DNPU_INPUT_ELECTRODE:
                    ramp_up = np.linspace(
                    ...
                    input_to_dnpu[j, :] = np.concatenate((
                        ramp_up, plateau, ramp_down
                    ))
                else:
                    input_to_dnpu[m, SLOPE_LENGTH + REST_LENGTH : -SLOPE_LENGTH] = input[i]
            tmp = DRIVER.forward_numpy(input_to_dnpu.T)
            tmp = tmp[SLOPE_LENGTH + REST_LENGTH : -SLOPE_LENGTH, 0]
            tmp = tmp - np.mean(tmp)
            tmp = butter_lowpass_filter(
                tmp, 5000, 4, 12500
            )
            # output.append(tmp)
            outputs[i][j * np.shape(input)[1]//10 : (j+1) * np.shape(input)[1]//10] = tmp[::10]
...
class UNET(nn.Module):
    def __init__(self) -> None:
        self.dnpu_layer = FourDNPUConvLayer()
        self.bn1 = nn.BatchNorm1d(4)

        self.linear_layer = nn.Linear(878, 878)
...
class DNPUClassifier(nn.Module):
    def __init__(self, n_input = 4, n_output = 10, n_channel = 32):
        super().__init__()
...
class FourDNPUConvLayer(nn.Module):
    def __init__(self, num_dnpus = NUM_DNPUs) -> None:
        super(FourDNPUConvLayer, self).__init__()
        self.dnpu_1 = DNPULayer()
        self.dnpu_2 = DNPULayer()
        self.dnpu_3 = DNPULayer()
        self.dnpu_4 = DNPULayer()

        # other possible solution
        self.dnpus = nn.ModuleList([DNPULayer() for i in range(num_dnpus)])

    def forward(self, x):
        x1 = self.dnpu_1(x).reshape(16, 1, 878)
        x2 = self.dnpu_2(x).reshape(16, 1, 878)
        x3 = self.dnpu_3(x).reshape(16, 1, 878)
        x4 = self.dnpu_4(x).reshape(16, 1, 878)
...
class MultipleDNPUClassifier(nn.Module):
    def __init__(self, num_dnpus) -> None:
        super(MultipleDNPUClassifier, self).__init__()
        self.num_dnpus = num_dnpus
        self.dnpu_layers = []
        for _ in range(0, num_dnpus):
            self.dnpu_layers.append(
                DNPULayer()
            )
        self.linear_layer = nn.Linear(int(878 * num_dnpus), 10) #878
    def forward(self, x):
        # x = x.reshape(x.size(0), -1)
        y = []
        for i in range(0, self.num_dnpus):
            y.append(
                self.dnpu_layers[i](x)
            )
        x = torch.cat((y), 1)
    ...
    def _get_params(self):
        ...
        for i in range(0, self.num_dnpus):
            params.append(
                self.dnpu_layers[i]._return_params()
            )
        return params
...
class OnlyConvLayer(nn.Module):
    def __init__(self) -> None:
        super(OnlyConvLayer, self).__init__()
...
def train(
    ...
    # loss_fn = CustomLossFunction(model, min = -0.45, max = 0.45)
