    ...
    if same_size_audios == None:
        ...
        if envelope == None:
            ...
            # data after low pass filtering are concatenated
                ...
                rand_matrix_of_cutoffs = np.linspace(4, 13, num=64, dtype=int)
                t = np.arange(0, (9710/12500), 1/12500)
                for i in range(len(dataset_numpy)):
                    for j in range(0, 64):
                        dataset_numpy_low_pass[i, max_length//10 * j : max_length//10 * (j+1)] = butter_lowpass_filter(
                        ...
                        #     data = dataset_numpy[i],
                        #     lowcut = int(rand_matrix_of_cutoffs[j]),
                        #     highcut = int(rand_matrix_of_cutoffs[j])+35,
                        #     fs = 12500,
                        #     order = 1
                        # )[::10]
                        ...
                        #     data = dataset_numpy[i],
                        #     lowcut = int(rand_matrix_of_cutoffs[j]),
                        #     highcut = int(rand_matrix_of_cutoffs[j])+25,
                        #     fs = 12500,
                        #     order = 3
                        # )[::10]
                        # ((0.5 * np.max(butter_lowpass_filter(
                        #                                                     data    = dataset_numpy[i],
                        #                                                     cutoff  = int(rand_matrix_of_cutoffs[j]),
                        #                                                     fs      = 12500,
                        #                                                     order   = 3)
                        ...
                        #                                                     fs      = 12500,
                        #                                                     order   = 3)
                        # )) * (np.tanh(dataset_numpy[i]/2)))[::10] + \
                        # ((0.5 * np.max(butter_lowpass_filter(
                        #                                                     data    = dataset_numpy[i],
                        #                                                     cutoff  = int(rand_matrix_of_cutoffs[j]),
                        #                                                     fs      = 12500,
                        #                                                     order   = 3)
                        # )) * ( dataset_numpy[i] * np.cos(2 * np.pi * 150 * t)))[::10]
