...
def load_dataset():
    ...
    spoken_mnist_train = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/bspytasks/spoken_digit_task/spoken_mnist/spoken_mnist/train"
    spoken_mnist_test = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/bspytasks/spoken_digit_task/spoken_mnist/spoken_mnist/test"
    ...
    for subdir, _, files in chain.from_iterable(os.walk(path) for path in dataset_paths):
        for file in files:
            if subdir[-5:] == "train":
                temp, _ = librosa.load(os.path.join(subdir, file), sr=None, mono = True, dtype=np.float32)

                max_peak = np.max(np.abs(temp))
                temp = temp * (1 / max_peak)

                dataset_train.append(temp)
                label_train.append(file[0])
            if subdir[-4:] == "test":
                temp, _ = librosa.load(os.path.join(subdir, file), sr=None, mono = True, dtype=np.float32)
