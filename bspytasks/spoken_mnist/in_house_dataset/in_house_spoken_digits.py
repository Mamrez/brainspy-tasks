...
import in_house_dataset_spoken_digits
...
class ConvLayer(torch.nn.Module):
    def __init__(self, num_classes, down_sample_no) -> None:
        ...
        self.fc1   = torch.nn.Linear(self.out_channel  * int((down_sample_no - self.kernel_size)/self.stride + 1), 64)
        self.relu_2 = torch.nn.ReLU()
...
class ProjectedAudioDataset(Dataset):
    def __init__(
                    ...
                    for i in range(0, num_projections):
                        ...
                        if top_projections != None:
                            ...
                            self.dataset_list.append(
                                np.append(
                                    # sklearn.preprocessing.minmax_scale(data-avg, (-1,1)) - np.mean(sklearn.preprocessing.minmax_scale(data-avg, (-1,1))),
                                    # 100*(data - avg),
                                    in_house_dataset_spoken_digits.butter_lowpass_filter(
                                        100*(data - avg),
                                        cutoff= 250,
                                        order = 10,
                                        fs = 24000
        ...
        #     if self.label_list[i] != '10' and self.label_list[i] != '00':
    ...
    for epoch in range(0, num_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            if i % (batch_size//4) == (batch_size//4 - 1):
                print("Loss after mini-batch %3d: %.3f" % (i+1, current_loss/(5 * batch_size//4)))
                current_loss = 0.
...
def measurement(
    ...
    dataset_train_val, label_train_val, dataset_test, label_test = in_house_dataset_spoken_digits.load_dataset()

    dataset_train_val, dataset_test = in_house_dataset_spoken_digits.audio_low_pass_filter(dataset_train_val, dataset_test)

    dataset_train_val = in_house_dataset_spoken_digits.pitch_shift(dataset_train_val)
    dataset_test = in_house_dataset_spoken_digits.pitch_shift(dataset_test)

    dataset_train_val = in_house_dataset_spoken_digits.normalize(dataset_train_val)
    dataset_test = in_house_dataset_spoken_digits.normalize(dataset_test)

    cnt = 0
    driver = get_driver(configs=configs["driver"])
    rand_matrix = np.random.uniform(-0.15, 0.15, size=(len(dnpu_control_indeces), num_projections))
    ...
    for d in range(len(dataset_train_val)):
        ...
        if d < 10:
            path_dataset = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/train/wilfred_pitch_shift_2/" + "data_" + str(0) +str(d)
            path_label = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/train/wilfred_pitch_shift_2/" + "label_" + str(0) + str(d)
        else:
            path_dataset = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/train/wilfred_pitch_shift_2/" + "data_" + str(d)
            path_label = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/train/wilfred_pitch_shift_2/" + "label_" + str(d)
    ...
    for d in range(len(dataset_test)):
        ...
        if d < 10:
            path_dataset = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/test/wilfred_pitch_shift_2/" + "data_" + str(0) + str(d)
            path_label = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/test/wilfred_pitch_shift_2/" + "label_" + str(0) + str(d)
        else:
            path_dataset = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/test/wilfred_pitch_shift_2/" + "data_" + str(d)
            path_label = "C:/Users/Mohamadreza/Documents/github/brainspy-tasks/tmp/projected_in_house_arsenic/test/wilfred_pitch_shift_2/" + "label_" + str(d)
